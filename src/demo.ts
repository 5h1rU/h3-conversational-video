...
function transitionTo(clipId){if(!clipId||clipId===currentClipId){renderList();return}const clip=playlist.entries.find(entry=>entry.id===clipId);if(!clip)return;if(playbackTimer!==null){clearTimeout(playbackTimer);playbackTimer=null}const image=q('#visual'),video=q('#generated-video'),isVideo=clip.mediaUrl.includes('/media/');currentClipId=clip.id;if(isVideo){image.style.display='none';video.style.display='block';if(video.getAttribute('src')!==clip.mediaUrl){video.src=clip.mediaUrl;video.currentTime=0}video.play().catch(()=>{q('#status').textContent='Video ready — press play'})}else{video.pause();video.style.display='none';image.style.display='block';image.src=clip.mediaUrl;playbackTimer=setTimeout(advance,clip.durationMs)}q('#speaker').textContent=clip.speaker;q('#title').textContent=clip.title;q('#generated').textContent=clip.source==='branch'?'PRIVATE ANSWER · RE-ENTRY INCLUDED':clip.source==='reentry'?'AUTOMATIC RE-ENTRY':'SHARED PROGRAM';renderList()}
function advance(){const clip=playlist.entries.find(entry=>entry.id===currentClipId);if(!clip)return;if(clip.source==='branch')completedBranchIds.add(clip.id);transitionTo(selectNextClipId(playlist.entries,clip.id,completedBranchIds))}
...
q('#generated-video').addEventListener('ended',()=>{const clip=playlist.entries.find(entry=>entry.id===currentClipId);if(clip?.mediaUrl.includes('/media/'))advance()});
