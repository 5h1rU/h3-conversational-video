...
async function refresh(){playlist=await api('/v1/sessions/'+sessionId+'/playlist');renderList()}
function renderList(){q('#timeline').innerHTML=playlist.entries.slice(Math.max(0,cursor-2),cursor+10).map((c,i)=>'<div class="clip '+c.source+' '+(i===Math.min(2,cursor)?'active':'')+'"><b>'+escapeHtml(c.source.toUpperCase())+'</b> · '+escapeHtml(c.title)+'</div>').join('');q('#meta').textContent='Session '+sessionId.slice(0,8)+' · playlist r'+playlist.revision+' · '+playlist.entries.length+' committed clips'}
function show(){if(!playlist.entries.length)return;const clip=playlist.entries[cursor%playlist.entries.length];q('#visual').src=clip.mediaUrl;q('#speaker').textContent=clip.speaker;q('#title').textContent=clip.title;q('#generated').textContent=clip.source==='branch'?'PRIVATE GENERATED BRANCH':clip.source==='reentry'?'AUTOMATIC RE-ENTRY':'SHARED PROGRAM';renderList()}
async function boot(){const created=await api('/v1/sessions',{method:'POST',headers:{'content-type':'application/json'},body:'{}'});sessionId=created.sessionId;q('#status').textContent='Canonical playback is continuous';await refresh();show();setInterval(()=>{cursor++;show()},5000);setInterval(refresh,2000);try{const protocol=location.protocol==='https:'?'wss:':'ws:';const ws=new WebSocket(protocol+'//'+location.host+'/v1/sessions/'+sessionId+'/ws');ws.onmessage=e=>{const m=JSON.parse(e.data);if(m.type==='playlist.revised')refresh();q('#status').textContent='Branch: '+m.state.branchPhase}}catch{}}
q('#ask').addEventListener('submit',async e=>{e.preventDefault();const input=q('#question');q('#status').textContent='Planning private response…';try{const result=await api('/v1/sessions/'+sessionId+'/events',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({eventId:crypto.randomUUID(),text:input.value,playbackPositionMs:cursor*5000,playlistRevision:playlist.revision})});q('#status').textContent='Branch: '+result.state.branchPhase;input.value=''}catch(error){q('#status').textContent=error.message}});boot().catch(e=>q('#status').textContent=e.message);
</script></body></html>`;
