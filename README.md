...
This repository is a deployable Cloudflare prototype of the first interactive “Signal Room” show. Its published shared episode is a ten-clip, 50-second sports-news program: Messi’s international retirement, the US Open, Mariano Navone’s upset of Novak Djokovic, Carlos Alcaraz’s return, and the Dutch Grand Prix. A viewer may create one private response package at the current semantic boundary; that package owns its natural ingress, answer, and egress, then resumes exactly once at the following canonical clip. Only validated, committed media may enter the playlist, and failure falls back to uninterrupted canonical playback.

The live sports episode was published atomically after all ten real five-second `480P` artifacts passed audiovisual and endpoint-frame continuity review. New sessions reuse those immutable D1/R2 catalog artifacts; partial or rejected builds are never exposed as the shared program. The six-clip expansion used exactly six paid provider jobs with no retries.
...
- Each accepted question is grounded by a bounded `kimi-k2.6` exchange with Moonshot's built-in `$web_search`. The first request may make exactly one search tool call and the second must return the final structured answer; the Worker does not run a browser or an open-ended agent loop. Only schema-validated HTTPS evidence is accepted. The grounded plan must name the original question subject, and its ingress must repeat that exact subject so a delayed branch remains understandable after the canonical program has moved to unrelated news. If the planner cannot produce an evidence-backed, subject-explicit answer, the branch fails safely before fal.ai is called and canonical playback continues.
- Topic placement is semantic without rewinding the viewer. Early Messi and US Open questions use their designed story boundaries; once playback has advanced beyond those boundaries, a branch stays at the current/next canonical boundary. The ordered branch owns exact natural ingress, grounded answer, and egress language, while the player follows playlist adjacency exactly and polling cannot pull it forward.
...
Personalized branches and continuity-sensitive canonical builds use the official `minimax/h3-max/image-to-video` endpoint. Canonical builds supply an explicit first-frame `image_url`. A personalized branch supplies both `image_url` and `end_image_url`: the validated Messi-context endpoint is also the first frame of the fixed US Open headline, so the generated exchange begins from and returns to that exact bridge pose. Canonical clips retain the versioned `h3-max-cost-first/1` five-second policy; personalized clips use `h3-max-conversational-branch/3` at seven seconds. Both remain `480P`, `balanced` prompt expansion, safety enabled, deterministic, and asynchronous. The branch reserves 1.2 seconds for ingress, four seconds for one concise factual answer, one second for egress, and 0.8 seconds of silent pose restoration. The complete spoken package is schema-limited to sixteen words and explicitly requires a calm 135–150 WPM cadence without acceleration or time compression. The adapter omits sync/base64 response fields. `src/fal-provider.ts` owns these typed contracts so provider defaults cannot silently change duration or resolution.

The versioned continuity contract `sports-news-continuity/1` fixes the fictional anchors, faces, wardrobe, studio, lighting, camera/lens/framing, voice identities, cadence, ambient audio, color grade, and chronological blocking. The first clip starts from [`assets/sports-news-visual-bible-v1.png`](assets/sports-news-visual-bible-v1.png); each of the other nine clips starts from the validated endpoint frame of its predecessor. D1 stores continuity and validation evidence and R2 holds the immutable source/endpoint images and MP4 manifests. The image-to-video route must serve both `HEAD` and `GET`, because fal probes the continuity asset before downloading it.
...
The planner is deliberately one bounded built-in search followed by one answer request, not an open-ended agent. Kimi thinking is explicitly disabled because Moonshot currently documents `$web_search` with non-thinking mode. The adapter receives the episode outline and viewer question as untrusted data, preserves Moonshot's opaque search arguments at the tool boundary, and then decodes a schema-validated topic, explicit question subject, confidence, exact ingress/answer/egress copy, information-as-of timestamp, and up to five HTTPS sources. An answer is rejected before fal spend if its ingress does not literally name the subject or if its combined spoken package exceeds sixteen words. The full plan and provider/model audit metadata are recorded in D1, but fal.ai receives only the resolved visual/dialogue specification.
...
fal webhooks are verified before JSON parsing using the four official signature headers, a five-minute timestamp window, raw-body SHA-256, and Ed25519 public keys from fal’s JWKS. A cached-key miss performs one fresh JWKS fetch and verification attempt so normal provider key rotation does not produce a false `401`. The signed request ID must match an expected D1 generation record. The explicit wire schema accepts fal’s JSON URL string and decodes it to an HTTPS `URL` on fal’s documented CDN only at the provider boundary; malformed or unsafe URL values receive a typed `422` response without logging the signed body or media URL. Webhook claims use `PROCESSING`, `RETRYABLE`, and `COMPLETED` states so transient ingestion failures can be redelivered without republishing completed work. fal media downloads use Workers' supported manual redirect mode and reject redirects before reading bytes. The seven-second branch profile retains a two-minute asynchronous generation deadline; canonical playback continues while it renders. A result after that deadline is acknowledged and recorded as `FAL_RESULT_AFTER_DEADLINE`, but is not downloaded, committed to R2, or inserted into the timeline.

Current primary references:

- [US Open official 2026 Monday results and highlights](https://www.usopen.org/en_US/news/articles/2026-08-31/who_won_on_monday_at_the_2026_us_open_results_and_highlights.html)
- [US Open official Navone–Djokovic report](https://www.usopen.org/en_US/news/articles/2026-08-30/navone_upsets_djokovic_in_2026_us_open_first-round_stunner.html)
- [Formula 1 official Dutch Grand Prix report](https://www.formula1.com/en/latest/article/norris-wins-dramatic-dutch-grand-prix-from-antonelli-and-russell-as-verstappen-crashes-out.Zn7iYevVGp5eHzFkTEAz7)
- [Reuters-derived report on Messi’s Argentina legacy](https://www.investing.com/news/world-news/soccerno-fairytale-ending-but-messi-leaves-defining-legacy-for-argentina-4883090)
- [Reuters-derived report on Messi’s international retirement](https://www.investing.com/news/world-news/messi-announces-departure-from-argentina-national-team-4883049)
...
The current prototype deployment was validated with remote session creation, Queue delivery, Durable Object publication, R2 artifact commit, revision-2 personalized playlist projection, full and byte-range media reads, and cross-session media denial. The reusable sports catalog was separately validated as ten distinct completed provider jobs, ten approved content-addressed R2 artifacts, one atomic D1 publication, and a fresh revision-1 session projected in the intended order. The six-clip expansion used six new provider jobs without retrying. The sports canonical builder additionally requires a `CANONICAL_ADMIN_TOKEN` Worker secret; it is never placed in source or configuration.

## Correctness and degradation

Tests cover deterministic canonical planning, the exact bounded Kimi `$web_search` exchange, sixteen-word pacing enforcement, repeated/unknown tool-call rejection, source and unsafe-URL rejection, response-size limits, topic-aware semantic placement, exact grounded dialogue compilation, the exact seven-second 480P branch request, five-second canonical preservation, voice-first UI and fallback, continuity-asset `HEAD`/`GET`, canonical reuse, atomic branch-package ordering, single-branch enforcement, duplicate viewer event/queue/webhook handling, failure fallback, Durable Object eviction/recovery, committed-only playlist projection, semantic re-entry ordering, immediate ready-branch queueing without canonical-loop delay, full canonical preloading, standby readiness, double-buffer handoff, player identity stability across polling, media-error continuation, and playlist revision changes. Grounding, provider, or deadline failure marks the branch failed without changing playlist revision, so canonical playback continues.
